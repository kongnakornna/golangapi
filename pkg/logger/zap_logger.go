...
// NewApiLogger สร้าง logger ด้วย config ที่ระบุ
func NewApiLogger(cfg *config.Config) Logger {
	return &apiLogger{cfg: cfg, sugarLogger: nil}
}

// NewLogger สร้าง logger โดยใช้ config จาก environment และ service name (ใช้เป็นชื่อ logger)
func NewLogger(serviceName string) Logger {
	cfg := config.GetCfg()
	log := NewApiLogger(cfg)
	log.InitLogger()
	// สามารถเพิ่ม service name ใน context ได้ (แต่ไม่บังคับ)
	return log
}
...
func (l *apiLogger) InitLogger() {
	...
	// ไม่ควร Sync ที่นี่ เพราะจะถูกเรียกตอน shutdown
}
