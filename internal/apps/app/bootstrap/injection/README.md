...
1. 在`internal/core/<domain>/repository`中定义新的仓库接口和实现
...
type Repositories struct {
    ...
    ProductRepo productrepo.ProductRepository  // 新增字段
}
...
func InitRepositories(db *gorm.DB, log logger.Logger) *Repositories {
    return &Repositories{
        ...
        ProductRepo: productrepo.NewProductRepository(db),  // 初始化新仓库
    }
}
```
...
1. 在`internal/core/<domain>/service`中定义新的服务接口和实现
2. 在`services.go`中的`Services`结构体添加新字段
3. 在`InitServices`函数中初始化新的服务实例

### 添加新的处理器

1. 在`internal/core/<domain>/handler`中定义新的处理器
