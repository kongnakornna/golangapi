...
| GET | `/api/v1/fullschedule/scheduleall` | รายการทั้งหมดแบบ settings-style (เหมือน `/settings/scheduleall`) |
| GET | `/api/v1/fullschedule/listschedulepage` | รายการแบบแบ่งหน้า settings-style (เหมือน `/settings/listschedulepage`) |
| GET | `/api/v1/fullschedule/scheduledevicepage` | รายการแมป schedule↔device แบบแบ่งหน้า (เหมือน `/settings/listscheduledevice`) |
...
### 4.1 รายการแบบ Settings-style (หน้าจอเดียวกับ `/settings/listschedulepage`)

Endpoint ใหม่ 3 ตัวให้ผลลัพธ์ **รูปประโยคเดียวกันกับโมดูล Settings เดิม** เพื่อให้ frontend
ใช้ component หน้า schedule เดิมได้เลย (เหมือน `http://localhost:4200/api/settings/listschedulepage?page=2&pageSize=10&start=`)

#### `GET /api/v1/fullschedule/listschedulepage`

Query params (เหมือน `/settings/listschedulepage` + ตัวกรองเพิ่มเติม):

| param | ความหมาย |
| :--- | :--- |
| `page` / `pageSize` | หน้า / จำนวนต่อหน้า (default 1 / 10, max 100; รองรับ alias `per_page`) |
| `start` | กรอง `time_start` (HH:MM) |
| `keyword` | ค้นหา `name` แบบ LIKE |
| `mode` | `normal` / `full` / `batch` |
| `status` | `active` / `inactive` |
| `event_type` | `device` / `email` |
| `event_action` | `ON` / `OFF` / `start` / `stop` |
| `group_id` / `zone_id` / `area_id` | กรองตามขอบเขต (uuid) |
| `sort` | `field-ASC` / `field-DESC` จาก whitelist (กัน SQL injection) |

คอลัมน์ที่รองรับ `sort`: `schedule_id`, `schedule_name`, `mode`, `status`, `time_start`/`start`,
`event_type`, `event_action`, `created_at`/`createddate`, `updated_at`/`updateddate`,
`next_run_at`, `last_run_at`

Response envelope (schema เดียวกับ settings):

```json
{
  "success": true,
  "data": {
    "page": 2, "currentPage": 2, "pageSize": 10, "totalPages": 3, "total": 25,
    "filter": { "start": "18:00" },
    "data": [
      { "schedule_id": "<uuid>", "schedule_name": "...", "mode": "normal",
        "status": "active", "time_start": "18:00", "event_type": "device",
        "event_action": "ON", "days_of_week": [2,3,4,5,6], "created_at": "...",
...
- `device_count` = จำนวนอุปกรณ์ที่แมปใน `fs_schedule_device` (คำนวณจาก grouped COUNT เดียว)
- แถวทุกแถวกรอง soft delete (`deleted_at IS NULL`) ออกแล้ว

#### `GET /api/v1/fullschedule/scheduleall`

รายการทั้งหมด (ไม่มีแบ่งหน้า) ตัวกรองชุดเดียวกับ `listschedulepage` —
ตอบกลับเป็น `[]` ของ schedule โดยตรง (เหมือน `/settings/scheduleall`)

#### `GET /api/v1/fullschedule/scheduledevicepage`

รายการแมป schedule↔device แบบแบ่งหน้า (เหมือน `/settings/listscheduledevice`):

| param | ความหมาย |
| :--- | :--- |
| `page` / `pageSize` | default 1 / 10, max 100 |
| `keyword` | ค้นหา `schedule.name` หรือ `device.device_name` |
| `schedule_id` | กรองตาม schedule (uuid) |
| `device_id` | กรองตาม device (int) |
| `status` | กรองตามสถานะของ schedule |
| `sort` | `schedule_id`, `schedule_name`, `start`, `device_id`, `device_name`, `status`, `created_at` |

แต่ละแถวคืน: `schedule_id`, `schedule_name`, `mode`, `status`, `start`, `event_action`,
`device_id`, `device_name`, `sn`

> เซิร์ฟเวอร์ mount ริงที่ `/api` (`server/handlers.go` line ~418) ดังนั้น path จริงคือ
> `/api/fullschedule/listschedulepage`, `/api/fullschedule/scheduleall`, `/api/fullschedule/scheduledevicepage`
...
## 9. การรวมกับระบบเดิม — `/api/settings/listschedulepage`

เพื่อให้หน้า schedule เดิมเห็น work จากโมดูลใหม่ด้วย `GET /api/settings/listschedulepage`
จึง **merge** ข้อมูล 2 แหล่งเข้าด้วยกันแล้วแบ่งหน้าใน Go:

1. `sd_iot_schedule` — ผ่าน `settings.ScheduleSpec` (เดิม)
2. `fs_schedule` — ผ่าน `settings.FullScheduleSpec` ใหม่ (ใน `specs_device_schedule.go`)
   ที่จัด `fs_schedule` ให้มี **column names เดียวกัน** กับ `sd_iot_schedule`:
   `schedule_id` (จาก `id::text`), `schedule_name` (จาก `name`), `start` (จาก `time_start`),
   `event` (จาก `event_action`), วันในสัปดาห์ `sunday..saturday` (จาก
   `days_of_week @> ARRAY[n]`), `status`, `createddate`/`updateddate`, และ `"countDevice"`
   (subquery นับ `fs_schedule_device`)

### วิธีรวม (ใน `ListSchedulePage` ของ settings)

```
RowsAll(ScheduleSpec)  +  RowsAll(FullScheduleSpec)   (ไม่แบ่งหน้าใน SQL)
        └────────── merge ──────────┘
normalize fs status: 'active'/'inactive' → '1'/'0'
convertDatesToBangkok()                  (เวลาใน 2 เซ็ตให้ format เดียวกัน)
sort ใน Go ตาม `sort` (default start ASC)
slice ด้วย page/pageSize → presenter.NewListResult
```

- **ตัวกรอง** (`keyword`, `start`, `event`, `status` ฯลฯ) ถูกประมวลผลใน SQL ของแต่ละสเปก
  (`status` ของ fs รองรับทั้ง `active/inactive` และ `1/0`)
- fs รายการที่ถูก **soft delete** จะถูกตัดออกด้วย `FixedFilter{Column: "deleted_at"}`
  ซึ่ง `SettingsPgRepo.listBase` ตีความค่า `Value == ""` เป็น `WHERE deleted_at IS NULL`
- `total` / `totalPages` คำนวณจากจำนวนแถวหลัง merge

### หมายเหตุ

- คอลัมน์ที่ตอบกลับของ fs บางคอลัมน์เป็น `null` (เช่น `device_id`, `device_name`) เพราะ
  fs_schedule ไม่มีคอลัมน์นั้นตรงๆ (device เก็บใน `fs_schedule_device`) — frontend ใช้
  `countDevice` แทน
- การตั้งค่าเรียกซ้ำๆ เช่น `?page=2&pageSize=10&start=18:00` ทำงานเหมือนเดิม (filters ยังกรองก่อน merge)